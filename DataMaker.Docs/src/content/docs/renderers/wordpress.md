...
The **FOBO Data Maker Forms** plugin hosts forms on a WordPress site. Upload a
...
2. Go to **FOBO Data Maker Forms → Upload .dmf** and upload a `.dmf` bundle. The
   plugin verifies the signature, stores the form, and lists it by id.
3. Place the form on a page:

   **Shortcode**
   ```text
   [fobo_data_maker_form id="contact_form"]
   ```

   The optional `theme` attribute (`on` / `off`) overrides theming per
   placement: `[fobo_data_maker_form id="contact_form" theme="off"]` renders
   structure-only so the active WP theme styles it.

   **Block** — add the *FOBO Data Maker Form* block (`fobo/data-maker-form`) and pick a
   form.
...
`FOBO_DATA_MAKER_FORMS_SYNC_API_URL` constant). A built-in submit proxy + upload-slot flow
...
Admin screens are gated by the `manage_fobo_data_maker_forms` capability **or**

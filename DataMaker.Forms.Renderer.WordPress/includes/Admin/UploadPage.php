...
use Fobo\DataMakerForms\DmfReader;
 ...
 * file + a chosen slug; SUCCESS persists the parsed form into wp_fobo_data_maker_forms
...
final class UploadPage
{
    ...
    {
        add_menu_page(
            __('FOBO Data Maker Forms', 'fobo-data-maker-forms'),
            __('FOBO Data Maker Forms', 'fobo-data-maker-forms'),
            'manage_options',
        ...
        add_submenu_page('fobo-data-maker-forms', __('Upload .dmf', 'fobo-data-maker-forms'), __('Upload .dmf', 'fobo-data-maker-forms'), 'manage_options', 'fobo-data-maker-forms',          [self::class, 'render']);
        add_submenu_page('fobo-data-maker-forms', __('Forms', 'fobo-data-maker-forms'),       __('Forms', 'fobo-data-maker-forms'),       'manage_options', 'fobo-data-maker-forms-forms',    [FormsListPage::class, 'render']);
        add_submenu_page('fobo-data-maker-forms', __('Settings', 'fobo-data-maker-forms'),    __('Settings', 'fobo-data-maker-forms'),    'manage_options', 'fobo-data-maker-forms-settings', [SettingsPage::class, 'render']);
    }
    ...
    public static function render(): void
    {
        ...
        ?>
        <div class="wrap">
            <?php PageHeader::render(__('FOBO Data Maker Forms — Upload .dmf', 'fobo-data-maker-forms')); ?>
            <?php if ($notice): ?>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><label for="dm-slug"><?php esc_html_e('Shortcode slug', 'fobo-data-maker-forms'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php
                                printf(
                                    ...
                                    esc_html__('Used in the shortcode: %s. Re-uploading the same slug overwrites the form in place.', 'fobo-data-maker-forms'),
                                    '<code>[fobo_data_maker_form id="customer-intake"]</code>'
                                );
                            ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="dm-file"><?php esc_html_e('.dmf bundle', 'fobo-data-maker-forms'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                ...
                                printf(
                                    ...
                                    esc_html__('Signature verification is currently %s — change under Settings.', 'fobo-data-maker-forms'),
                                    '<strong>' . ($settings['verify_signature'] ? esc_html__('ON', 'fobo-data-maker-forms') : esc_html__('OFF', 'fobo-data-maker-forms')) . '</strong>'
                                );
                                ?>
                            </p>
                        </td>
    ...
    private static function handle_upload(): array
    {
        ...
        if (empty($_POST['dm_upload_nonce'])
            ...
            return ['kind' => 'error', 'html' => esc_html__('Security check failed. Please try again.', 'fobo-data-maker-forms')];
        }
        ...
        if ($tmp_name === '' || !is_uploaded_file($tmp_name)) {
            return ['kind' => 'error', 'html' => esc_html__('No file uploaded.', 'fobo-data-maker-forms')];
        }
        $slug = isset($_POST['slug']) ? sanitize_title(wp_unslash($_POST['slug'])) : '';
        if (!$slug) {
            return ['kind' => 'error', 'html' => esc_html__('Slug is required.', 'fobo-data-maker-forms')];
        }

        $max_bytes = (int)apply_filters('fobo_data_maker_forms_max_dmf_bytes', self::MAX_DMF_BYTES_DEFAULT);
        ...
        if ($max_bytes > 0 && $file_size > $max_bytes) {
            return [
                ...
                'html' => sprintf(
                    ...
                    esc_html__('Uploaded .dmf is larger than the %s limit.', 'fobo-data-maker-forms'),
        ...
        if ($bytes === false) {
            return ['kind' => 'error', 'html' => esc_html__('Could not read the uploaded file.', 'fobo-data-maker-forms')];
        }
        $settings = SettingsPage::get();
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[fobo-data-maker-forms] .dmf parse failed: ' . $e->getMessage()); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- gated behind WP_DEBUG; diagnostic for the admin upload flow.
            }
            return ['kind' => 'error', 'html' => esc_html__('Could not parse the .dmf bundle (signature, format, or signing-key mismatch).', 'fobo-data-maker-forms')];
        }
        ...
        if (!$hasRecipient) {
            return [
                'kind' => 'error',
                'html' => esc_html__('This .dmf was exported in share-only mode (no recipient block). Submissions can\'t route back to a publisher, so the plugin won\'t accept it. Sign in to FOBO in the Data Maker desktop app, re-export the form, and try again.', 'fobo-data-maker-forms'),
            ];
        }

        $use_theme = !empty($_POST['use_theme']);
        $id = FormStore::upsert($bundle, $slug, get_current_user_id(), $use_theme);
        $shortcode = '[fobo_data_maker_form id="' . esc_attr($slug) . '"]';
        return [
            'kind' => 'success',
            'html' => sprintf(
                /* translators: 1: numeric form id, 2: shortcode */
                esc_html__('Form saved (id #%1$d). Embed it with: %2$s', 'fobo-data-maker-forms'),
                $id,
